...
function [Msg, iterations] = My_ML_LDPC_Erasure_Decoder(recv_vec_val, G, k)
...
% Let's represent an erasure by '-1'
    ...
    bottom_mat_counter = num_recv_symbols; % Keep track of where all zeros rows have been swapped to
    NotDone = 1;
    while (row_index <= k) && (NotDone == 1)
        ...
        if (GJ_mat(row_index, row_index) ~= 0) % since binary, must be '1'
            ...
            if (swap_ind > bottom_mat_counter) % we have run out of rows to choose from
                NotDone = 0;
            else
                if (sum(GJ_mat(row_index,:)) == 0) %swap with bottom row
                    % The all zeros row at row_index can be ignored, so
                    % don't need to actually swap, just move
                    % bottom_mat_counter row to row_index position
                    GJ_mat(row_index, :) = GJ_mat(bottom_mat_counter, :);
                    repair_multiply_accumulator(row_index) = repair_multiply_accumulator(bottom_mat_counter);
                    bottom_mat_counter = bottom_mat_counter - 1;
                else
